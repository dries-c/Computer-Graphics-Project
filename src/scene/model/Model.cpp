...
Model::Model(const std::vector<glm::mat4> &modelMatrices, Shader shader, const std::vector<Mesh> &meshes) : modelMatrices(modelMatrices), shader(shader), meshes(meshes) {
    if (modelMatrices.size() > 1) {
        setupInstancing();
    }
}
